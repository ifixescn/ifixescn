...
interface PageMetaProps {
  ...
  fullTitleOnly?: boolean;
  /** 禁止搜索引擎索引（用于登录、注册等私有页面） */
  noIndex?: boolean;
  /** 覆盖 canonical URL，默认使用当前路径 */
  canonicalUrl?: string;
}
 ...
 * 
 * 不使用后缀：
 * <PageMeta title="Exact Title" fullTitleOnly={true} />
 * 
 * 自动生成描述：
 * <PageMeta 
 *   title="Product Name"
...
const PageMeta = ({ 
  ...
  // 构建完整标题
  let fullTitle = title;
  if (!fullTitleOnly && title) {
    fullTitle = `${title} - ${settings.siteName}`;
  } else if (!title) {
    fullTitle = settings.siteName;
  }
  ...
  return (
    <Helmet>
      ...
      {/* hreflang 多语言支持 */}
      {!noIndex && HREFLANG_LANGS.map(({ code }) => (
        <link
          key={code}
          rel="alternate"
          hrefLang={code}
          href={`${SITE_DOMAIN}${currentPath}`}
        />
      ))}
      {!noIndex && (
        ...
        <script type="application/ld+json">
          {JSON.stringify({
            ...
            "publisher": {
              ...
              "logo": {
                "@type": "ImageObject",
                "url": settings.siteLogo || `${SITE_DOMAIN}/favicon.png`
              }
            },
